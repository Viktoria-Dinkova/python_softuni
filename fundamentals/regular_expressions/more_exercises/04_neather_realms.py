...
# from which only one will survive. # Your job, however is not so exciting. You are assigned to
# sign in all the participants in the nether realm's mighty battle's demon book, which of course is sorted alphabetically.
...
# Input
# The input will be read from the console. The input consists of a single line containing all demon names separated by commas and zero or more spaces
...
participants = [x.replace(' ', '') for x in participants]
participants = sorted(participants)
def find_name(curr_participant: str):
    pattern = r'([^\d\+\-\*\/\.]+)'
    for letter in curr_participant:
        name = re.findall(pattern, curr_participant)
    return ''.join(name)


def find_numbers(curr_participant: str):
    pattern = r'([+-]*\d*[\.\d]*)'
    for letter in curr_participant:
        numbers = re.findall(pattern, curr_participant)
        numbers = [float(x) for x in numbers if len(x) > 0]
    return numbers


def find_actions(curr_participant: str):
    pattern = r'([\*//]*)'
    for letter in curr_participant:
        action = re.findall(pattern, curr_participant)
        action = [x for x in action if len(x) > 0]
    return ''.join(action)

for curr_demon in participants:
    health = 0
    damage = 0
    name = find_name(curr_participant=curr_demon)
    nums = find_numbers(curr_participant=curr_demon)
    acct = find_actions(curr_participant=curr_demon)
    if name:
        health = sum([ord(x) for x in name])
    if nums:
        damage = sum(nums)
        if acct:
            for i in acct:
    ...
    print(f'{curr_demon} - {health} health, {damage:.2f} damage ')
