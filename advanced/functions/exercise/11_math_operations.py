...
def math_operations(*float_numbers, **kwargs):
    index = 0
    floats = deque(float_numbers)
    while floats:
        if floats:
            kwargs['a'] += floats.popleft()
        else:
            break

        if floats:
            kwargs['s'] -= floats.popleft()
        else:
            break

        if floats:
            if floats[0] != 0:
                kwargs['d'] /= floats.popleft()
            else: floats.popleft()
        else:
            break

        if floats:
            kwargs['m'] *= floats.popleft()
        else:
            break

    return '\n'.join(f"{el[0]}: {el[1]:.1f}" for el in sorted(kwargs.items(), key=lambda item: (-item[1], item[0])))
